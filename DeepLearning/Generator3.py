...
def train(train_path, valid_path, test_path, preprocess=1, batch_size=32, epochs=5, verbose=1, img_size=-1,
    ...
    # -------- New Change: Added Throttle to label data for training set ---------------
    if all_train_test == 0 or all_train_test == 1:
        for name in glob.glob1(train_path, '*jpg'):  # Training get data
            ...
            steering = float(label[4])
            throttle = float(label[6])

            label_data = [steering, throttle]
            y_labels.append(label_data)

        # -------- New Change: Added Throttle to label data for validation set ---------------
        for name_y in glob.glob1(valid_path, '*jpg'):  # Validation get data
            ...
            label_y = label_y.split('_')

            steering_val = float(label_y[4])
            throttle_val = float(label_y[6])

            label_y_data = [steering_val, throttle_val]
            val_labels.append(label_y_data)
    ...
    X, y = valid_imagess, np.array(val_labels)     # New change: Converting val_labels from list to np.array
...
if __name__ == '__main__':
    ...
    train(train_path=train_path, valid_path=valid_path, test_path=test_path, preprocess=1, batch_size=64, epochs=5,
          verbose=1, img_size=-1, checkpoint=check,
